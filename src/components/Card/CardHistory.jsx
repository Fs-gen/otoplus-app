  ...
  return (
    ...
    >
      <div className="flex gap-2 flex-2">
        <div className="p-1.5 flex bg-blue-semi h-max rounded-full">{icon}</div>
        <div className="flex flex-col gap-1.5 text-xs capitalize">
          ...
          {props.type == "penjualan" ? <span className="py-2"></span> : (
      ...
      {status ? (
        <h1
          className={`${status} text-xs text-right font-semibold capitalize flex-1`}
        >
          {props.status}
        </h1>
      ) : null}
